...
impl MakerExecutor {
    ...
    pub async fn execute(
        ...
        let params_val: Value = serde_json::from_str(params_json).map_err(|e| {
            SyscallError::InternalError(format!("Invalid params JSON: {}", e))
        })?;

        let params_js = JsValue::from_json(&params_val, &mut context)
            .map_err(|e| SyscallError::InternalError(format!("Failed to convert params to JS: {}", e)))?;
        ...
        let _ = context.register_global_builtin_callable(
            ...
            boa_engine::native_function::NativeFunction::from_copy_closure(|_this, args, ctx| {
                ...
                let tid_val = ctx.global_object().get(boa_engine::js_string!("__TENANT_ID__"), ctx)
                    .map_err(|e| JsError::from_opaque(JsValue::from(boa_engine::js_string!(e.to_string()))))?;
                
                let tid = tid_val.as_string()
                    .map(|s| s.to_std_string().unwrap_or_default())
                    .unwrap_or_else(|| "default".to_string());

                if path_str.contains("..") || path_str.starts_with("/") || path_str.starts_with("\\") {
                    return Ok(JsValue::from(boa_engine::js_string!("Security Error: Path traversal blocked")));
                }

                let full_path = Path::new("./users").join(&tid).join("workspace").join(&path_str);
                match std::fs::read_to_string(full_path) {
                    Ok(content) => Ok(JsValue::from(boa_engine::js_string!(content))),
                    Err(e) => Ok(JsValue::from(boa_engine::js_string!(format!("IO Error: {}", e)))),
                }
            })
        );
        ...
        match context.eval(Source::from_bytes(code)) {
            Ok(res) => {
                let js_str = res.to_string(&mut context)
                    .map_err(|e| SyscallError::InternalError(format!("Result conversion failed: {}", e)))?;
                let out = js_str.to_std_string()
                    .map_err(|e| SyscallError::InternalError(format!("UTF-8 conversion failed: {}", e)))?;
                Ok(out)
            }
            Err(e) => {
                Err(SyscallError::InternalError(format!("JS Error: {}", e.to_string())))
            }
        }
    }
}
...
mod tests {
    ...
    async fn test_maker_filesystem_jail() {
        ...
        let res_escape = maker.execute(tenant_id, "js", code_escape, "{}").await.unwrap();
