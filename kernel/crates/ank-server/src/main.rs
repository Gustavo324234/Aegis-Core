...
/// asegúrese de que las variables de entorno requeridas estén correctamente inicializadas.
fn load_env_file() {
...
pub(crate) async fn run_server() -> Result<()> {
    ...
    // Citadel: Auto-generate plugin signing keys if not present
    let keys_dir = data_dir.join("keys");
    let pub_key_path = keys_dir.join("plugin_signer.pub");
    let priv_key_path = keys_dir.join("plugin_signer.key");

    if std::env::var("AEGIS_PLUGIN_ROOT_KEY").is_err() {
                ...
                tokio::spawn(async move {
                        ...
                        .await
                    {
                        Ok(stream) => {
                            ...
                            // an empty request.
                            let output_is_empty = (full_output.trim().is_empty()
                                || tokens_emitted == 0)
                                && !tool_calls_detected;
                            if tokens_emitted == 0 {
                                tracing::warn!(
